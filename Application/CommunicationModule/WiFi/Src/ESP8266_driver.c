	...
	while ( count <= 10 )
	{
		if( ESP8266_Cmd ( "AT", "OK", NULL, 500 ) ){
			if(!clear_alarm_and_count(1, &register_data_warning_store.InitAlarm, register_address_warning_store.InitAlarm, 1,
				&register_data_warning_store.InitResetCount, register_address_warning_store.InitResetCount, 1)) return;
		}
		ESP8266_Rst();
		++ count;
 ...
 * 调用  ：被外部调用
 */
bool ESP8266_UDP_Link_Server ( ENUM_NetPro_TypeDef enumE, char * ip, char * ComNum, char * RemoteComNum, ENUM_ID_NO_TypeDef id)
{
	char cStr [100] = { 0 }, cCmd [120];
  ...
  {
		case enumTCP:
		  sprintf ( cStr, "\"%s\",\"%s\",%s,%s,0", "TCP", ip, ComNum, RemoteComNum );
		  break;
		
		case enumUDP:
		  sprintf ( cStr, "\"%s\",\"%s\",%s,%s,0", "UDP", ip, ComNum, RemoteComNum );
		  break;
...
//	macESP8266_Usart ( "+++" );
	HAL_UART_Transmit_IT(&huart3,"+++",sizeof("+++"));
